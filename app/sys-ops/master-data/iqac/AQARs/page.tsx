// app/sys-ops/master-data/iqac/AQARs/page.tsx
"use client";
...
export default function AqarAdminPage() {
    ...
    const [editForm, setEditForm] = useState<Partial<AqarDoc>>({});
    ...
    const fetch_ = async () => {
        setLoading(true);
        try {
            const r = await fetch("/api/iqac/AQARs?includeInactive=true");
            const d = await r.json();
            if (d.success) setDocs(d.data || []);
        } catch {
            toast.error("Failed to load AQAR documents");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const r = await fetch("/api/iqac/AQARs/upload", { method: "POST", body: fd });
            const d = await r.json();
            if (d.success) {
                toast.success("AQAR document uploaded");
                setShowForm(false);
                setFile(null);
                setForm(emptyForm);
                fetch_();
    ...
    const handleUpdate = async (id: number) => {
        try {
            const r = await fetch("/api/iqac/AQARs", {
                method: "PUT",
    ...
    const handleDelete = async (id: number) => {
        if (!confirm("Delete this AQAR document? This cannot be undone.")) return;
        try {
            const r = await fetch(`/api/iqac/AQARs?id=${id}`, { method: "DELETE" });
            const d = await r.json();
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                <h1 className="text-3xl font-bold text-gray-900">AQAR Document Management</h1>
                <p className="text-gray-600 mt-1">
                    Manage Annual Quality Assurance Report (AQAR) documents for IQAC
                </p>
            </div>
            ...
            <div className="flex justify-between items-center">
                ...
                >
                    <Plus className="w-4 h-4" /> Upload AQAR Document
                </button>
            </div>
                        ...
                        <div>
                            ...
                            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-[#342D87] transition-colors cursor-pointer">
                                <input
                                    ...
                                    id="aqar-file-upload"
                                />
                                <label htmlFor="aqar-file-upload" className="cursor-pointer flex flex-col items-center">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    placeholder="Brief description of this AQAR..."
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                />
                                <label htmlFor="aqar_active" className="text-sm text-gray-700">Active</label>
                            </div>
                        </div>
                    </div>
                ...
                <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {loading ? (
                            ...
                            <tr><td colSpan={8} className="text-center py-8 text-gray-400">No AQAR documents yet. Upload the first one.</td></tr>
                        ) : docs.map((doc) => (
