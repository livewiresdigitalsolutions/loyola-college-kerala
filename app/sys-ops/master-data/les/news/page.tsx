...
export default function LesNewsPage() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (res.ok) { toast.success("News added"); setFormData({ title: "" }); setIsAdding(false); fetchItems(); }
  ...
  const handleUpdate = async (id: number) => {
    ...
    try {
      ...
      if (res.ok) { toast.success("Updated"); setEditingId(null); setFormData({ title: "" }); fetchItems(); }
  ...
  const startEdit = (item: NewsItem) => { setEditingId(item.id); setFormData({ title: item.title }); setIsAdding(false); };
  const cancelEdit = () => { setEditingId(null); setIsAdding(false); setFormData({ title: "" }); };
  ...
  const renderForm = (onSave: () => void) => (
    <div className="flex flex-col gap-3">
      ...
      <p className="text-xs text-gray-500">Timestamp will be calculated automatically from the time of posting.</p>
    ...
    <>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => { setIsAdding(true); setEditingId(null); setFormData({ title: "" }); }} className="flex items-center gap-2 px-4 py-2 bg-[#342D87] text-white rounded-lg hover:bg-[#2a2470] transition-colors"><Plus className="w-4 h-4" />Add News</button>
        </div>
          ...
          : (<div className="space-y-3">{items.map((item) => (
            <div key={item.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
              {editingId === item.id ? (<div className="flex-1">{renderForm(() => handleUpdate(item.id))}</div>) : (<>
                <div><p className="font-semibold text-gray-900">{item.title}</p><p className="text-sm text-gray-500">{item.timeAgo || 'Just now'}</p></div>
