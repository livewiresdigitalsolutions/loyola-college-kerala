// app/api/iqac/ssr/upload/route.ts
...
export async function POST(request: Request) {
    try {
        ...
        const uploadDir = path.join(process.cwd(), "public", "iqac", "ssr-docs");
        ...
        const fileUrl = `/iqac/ssr-docs/${fileName}`;
        ...
        try {
            ...
            return NextResponse.json({
                success: true,
                message: "SSR document uploaded successfully",
        ...
        console.error("Error uploading SSR document:", error);
        return NextResponse.json(
            { success: false, error: "Failed to upload SSR document", details: error.message },
            { status: 500 }
        );
    }
}
