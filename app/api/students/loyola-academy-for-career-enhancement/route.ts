// app/api/students/loyola-academy-for-career-enhancement/route.ts
...
type LaceType = "organizing-team" | "activities" | "achievements" | "gallery";

const tableMap: Record<LaceType, string> = {
    "organizing-team": "lace_organizing_team",
    "activities": "lace_activities",
...
export async function GET(request: Request) {
    try {
        ...
        const type = searchParams.get("type") as LaceType;
...
export async function DELETE(request: Request) {
    try {
        ...
        const imgTypes: LaceType[] = ["organizing-team", "achievements", "gallery"];
        ...
        if (isDevelopment) {
            if (imgTypes.includes(type)) {
                ...
                if (rec?.image_url) {
                    ...
                    if (fn) await supabase.storage.from("lace").remove([`${type}/${fn}`]);
                }
            }
