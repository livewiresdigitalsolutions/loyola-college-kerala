// app/api/students/loyola-initiative-for-language-advancement/route.ts
...
type LilaType = "organizing-team" | "activities" | "topics" | "selection-process" | "gallery";

const tableMap: Record<LilaType, string> = {
    "organizing-team": "lila_organizing_team",
    "activities": "lila_activities",
    "topics": "lila_topics",
    "selection-process": "lila_selection_process",
    "gallery": "lila_gallery",
};

export async function GET(request: Request) {
    try {
        const { searchParams } = new URL(request.url);
        const type = searchParams.get("type") as LilaType;
    ...
    } catch (e: any) { return NextResponse.json({ success: false, error: e.message || "Failed to fetch" }, { status: 500 }); }
}
...
export async function POST(request: Request) {
    ...
    } catch (e: any) { return NextResponse.json({ success: false, error: e.message || "Failed to create" }, { status: 500 }); }
}
...
export async function PUT(request: Request) {
    ...
    } catch (e: any) { return NextResponse.json({ success: false, error: e.message || "Failed to update" }, { status: 500 }); }
}
...
export async function DELETE(request: Request) {
        ...
        if (isDevelopment) {
            if (type === "organizing-team" || type === "gallery") {
                ...
                if (rec?.image_url) {
                    ...
                    if (fn) await supabase.storage.from("lila").remove([`${type === "organizing-team" ? "team" : "gallery"}/${fn}`]);
                }
            }
    ...
    } catch (e: any) { return NextResponse.json({ success: false, error: e.message || "Failed to delete" }, { status: 500 }); }
}
