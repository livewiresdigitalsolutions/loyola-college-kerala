...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const idNum = parseInt(id);
        if (isDevelopment) {
            const { error } = await supabase.from('les_coordinators').delete().eq('id', idNum);
            if (error) throw error;
        } else {
            const connection = await mysql.createConnection(mysqlConfig);
            await connection.execute('DELETE FROM les_coordinators WHERE id = ?', [idNum]);
            await connection.end();
        }
        return NextResponse.json({ message: 'Coordinator deleted successfully' });
    } catch (error: any) {
        return NextResponse.json({ error: 'Failed to delete coordinator', details: error.message }, { status: 500 });
    }
}
