...
import { Loader2, Save, Send, ArrowLeft, Upload, X, ImageIcon } from "lucide-react";
...
export default function NewArticlePage() {
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      const res = await fetch("/api/journals/articles", {
        method: "POST",
      ...
      if (res.ok && data.success) {
        ...
        setTimeout(() => router.push("/journals/dashboard/articles"), 1000);
      } else {
  ...
  const handleSaveAndSubmit = async () => {
    ...
    if (!form.abstract.trim()) {
      toast.error("Abstract is required for submission");
      return;
    }
    ...
    try {
      ...
      const createRes = await fetch("/api/journals/articles", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(form),
      });
      ...
      if (!createRes.ok || !createData.success) {
        toast.error(createData.error || "Failed to save");
        return;
      }

      // Then submit it
      const submitRes = await fetch(`/api/journals/articles/${createData.article.id}/submit`, {
        method: "POST",
      });
      ...
      if (submitRes.ok && submitData.success) {
        ...
        toast.error(submitData.error || "Failed to submit");
      }
    } catch {
      toast.error("Network error");
    } finally {
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8 space-y-6">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              placeholder="Brief summary of your article (required for submission)"
          ...
          <div className="flex items-center gap-3 pt-4 border-t border-gray-100">
            <button
              ...
              className="flex items-center gap-2 bg-gray-100 text-foreground px-6 py-3 rounded-lg text-sm font-medium hover:bg-gray-200 transition-colors disabled:opacity-50"
            >
              {isSubmitting ? (
